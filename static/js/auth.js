...
const profilePhotoCameraInput = document.getElementById("profile-photo-camera");
const photoCameraButton = document.getElementById("photo-camera-btn");
...
function persistProfilePhoto(file) {
    ...
    reader.onload = () => {
        try {
            localStorage.setItem("wavechat-profile-photo", reader.result);
        } catch (_error) {
...
function updateCountryUI() {
    ...
    if (countryFlag) {
        countryFlag.textContent = resolvedFlag;
    }
    if (countryName) {
    ...
    if (phoneFlag) {
        phoneFlag.textContent = resolvedFlag;
    }
    if (phoneDial) {
...
function updateNameValidation() {
    ...
    displayNameInput?.classList.toggle("is-active", value.length > 0);
    if (nameContinueButton) {
        nameContinueButton.disabled = !valid;
    }
}
...
async function requestOtp() {
    ...
    try {
        ...
        state.requiresProfileSetup = data.hasAccount === false;
        if (data.hasAccount) {
            setStatus("Number verified. Welcome back.");
        } else {
...
async function verifyOtpAndContinue() {
    ...
    if (state.requiresProfileSetup) {
        setStatus("Code accepted. Finish your profile to continue.");
        setStep("name");
        return;
    }
...
function selectCountryFromButton(button) {
    ...
    state.country = {
        ...
        flag: button.dataset.flag || "ðŸ³ï¸",
        length: Number(button.dataset.length || 10),
    };
    if (flagText) {
        state.country.flag = flagText;
    }
...
function attachListeners() {
    ...
    profilePhotoCameraInput?.addEventListener("change", (event) => {
        const file = event.target.files?.[0];
        handlePhotoSelection(file);
    });
